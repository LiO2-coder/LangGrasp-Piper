...
def main() -> None:
    ...
    try:
        logs.event("demo_start", "文字输入 demo 启动，正在加载 MuJoCo 模型。", scene_xml=str(scene_xml))
        ...
        logs.exception(f"文字输入 demo 异常退出：{exc}", exc=exc, event="demo_failed", error=str(exc))
        raise
    finally:
        try:
            try:
            ...
            logs.event("demo_shutdown", "文字输入 demo 资源已释放。")
        finally:
            logs.close()
